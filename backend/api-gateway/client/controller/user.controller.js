      ...
      ConfirmPassword :req.body.ConfirmPassword
                ...
                if (requestObject.OldPassword || requestObject.NewPassword || requestObject.ConfirmPassword) {
                  JWT.verify(token , secretKey, (err, decoded) =>{
                    ...
                    // if (!decode) {
                    //   res.status(404).send("Unauthorized user, Please provide token");
                    // }else{
                    //   client.updatePassword((requestObject),(error, response) => {
                    //     if (error) {
                    //       //console.error("Error listing records:", error);
            ...
            exports.updateUserPIN = async (req, res) => {
              ...
              if(requestObject.OldPIN_Number && requestObject.NewPIN_Number && requestObject.ConfirmPIN_Number){
                JWT.verify(token , secret_key, (err, decoded) =>{
                  if(err){
                    ...
                    client.updatePIN(requestObject, (error, response) => {
                      if (error) {
                        //console.error("Error listing records:", error);
                        res.status(400).send(error);
                      } else {
