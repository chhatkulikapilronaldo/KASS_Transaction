...
(() => {
  ...
  module.exports = async (call) => {
    ...
    try {
      ...
      connection = await dbHelper.getConnection();
      const balanceUpdate = await connection.query(`Update deposits set Amount = Amount - ? where Account_Number = ? LIMIT 1;`,[insert.Amount,insert.Sender_Account],(err,result)=>{
        if(err){
          connection.rollback(()=>{
            throw err;
          });
