  ...
  module.exports = async (call) => {
    ...
    try {
      ...
      const [output] = await connection.query(
        `select * FROM login_infos WHERE Password='${OldPassword}'`
      );
      ...
      const compare = await authenticationHelper.authenticate(
        OldPassword,
        storedPassword
      );
      ...
      if (compare == true) {
        if (NewPassword == ConfirmPassword) {
          ...
          const change = await connection.query(
            `Update login_infos set Password='${NewPassword}' where uuid='${uuid}'`
          );
          const [rows] = await connection.query(
            `Update users set Password='${hashedPassword}', Confirm_Password='${hashedPassword}', updatedAt='${date}' where uuid='${uuid}'`
          );
      ...
      let response = {
        status: httpStatus.BAD_REQUEST,
        message: "Old Password and Stored Password doesnt match",
      };
      return response;
    } finally {
